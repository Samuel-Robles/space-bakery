...
public class BeEaten : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Y) && moonExists)
        {
            //print("Y key was pressed");
            GameObject objectToDisappear = GameObject.Find("moonCake");
            objectToDisappear.GetComponent<Renderer>().enabled = false;
            objectToDisappear = GameObject.Find("Y");
            objectToDisappear.GetComponent<Renderer>().enabled = false;
            audioSource.Play();
            moonExists = false;
        }
